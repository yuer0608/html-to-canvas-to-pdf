...
export function downLoadPdf(props: { dom: Element; pdfName: string; styles?: { [k in string]: string } }) {
    const { dom, pdfName, styles } = props;
    return new Promise((resolve, reject) => {
        const content = document.createElement('div');
        content.style.position = styles?.position || 'absolute';
        content.style.zIndex = styles?.zIndex || '-1';
        content.style.top = styles?.top || '0';
        content.style.margin = styles?.margin || 'auto';
        content.style.width = styles?.width || '895px';
        content.style.padding = styles?.padding || '20px';
        content.style.color = styles?.color || 'rgb(34, 47, 62)';
        content.style.fontFamily = styles?.fontFamily || 'Helvetica, Arial, sans - serif';
        content.innerHTML = dom.innerHTML;
